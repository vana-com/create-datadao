...
describe('Config Functions', () => {
  ...
  describe('setupConfig', () => {
    ...
    test('validates private key input', async () => {
      ...
      inquirer.prompt.mockImplementation((questions) => {
        const question = Array.isArray(questions) ? questions.find(q => q.name === 'privateKey') : questions;
        if (question && question.validate) {
          // Test the validation function
          const firstResult = question.validate('invalid-key');
          expect(firstResult).toBe('Invalid private key format');
          
          const secondResult = question.validate('0x1234567890123456789012345678901234567890123456789012345678901234');
          expect(secondResult).toBe(true);
        }
        
        return Promise.resolve({
          [question.name]: '0x1234567890123456789012345678901234567890123456789012345678901234'
        });
      });
    ...
    test('displays correct console messages', async () => {
      inquirer.prompt.mockResolvedValue({
        dlpName: 'TestDAO',
        tokenName: 'TestToken',
        tokenSymbol: 'TEST',
