...
async function generateTemplate(targetDir, config) {
  ...
  try {
    ...
    try {
      ...
      spinner.text = 'Root dependencies installed';

      // Install contract dependencies
      execSync('npm install', { cwd: path.join(targetDir, 'contracts'), stdio: 'pipe' });
      spinner.text = 'Contract dependencies installed';

      // Install UI dependencies
      execSync('npm install', { cwd: path.join(targetDir, 'ui'), stdio: 'pipe' });
      spinner.text = 'All dependencies installed successfully';
    } catch (error) {
      spinner.warn('Failed to install some dependencies automatically');
      console.log(chalk.yellow('You may need to run:'));
      console.log(chalk.yellow('  cd ' + path.basename(targetDir)));
      console.log(chalk.yellow('  npm install'));
      console.log(chalk.yellow('  cd contracts && npm install'));
      console.log(chalk.yellow('  cd ../ui && npm install'));
    }
